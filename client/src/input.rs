...
impl AppInput {
    ...
    ) -> BoxedResult<ControlFlow<Option<String>>> {
        terminal.draw(|f| self.ui(f))?;
        Ok(self.handle_events()?)
    }

    pub fn handle_events(&mut self) -> BoxedResult<ControlFlow<Option<String>>> {
            ...
            if let Event::Key(key) = event::read()? {
                match self.input_mode {
                    ...
                    InputMode::Editing => match key.code {
                        KeyCode::Enter => {
                            let result: String = self.input.value().into();
                            self.input.reset();
                            return Ok(ControlFlow::Break(Some(result)));
                        }
                        KeyCode::Esc => {
